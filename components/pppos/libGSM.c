...
static void pppos_client_task()
{
    ...
    if (gpio_set_direction(GSM_GPIO_TX, GPIO_MODE_OUTPUT)) goto exit;
	if (gpio_set_direction(GSM_GPIO_RX, GPIO_MODE_INPUT)) goto exit;
	if (gpio_set_pull_mode(GSM_GPIO_RX, GPIO_PULLUP_ONLY)) goto exit;

	char PPP_ApnATReq[sizeof(CONFIG_GSM_APN)+24];
	
	uart_config_t uart_config = {
			.baud_rate = GSM_UART_BDRATE,
	...
	if (uart_set_pin(uart_num, GSM_GPIO_TX, GSM_GPIO_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE)) goto exit;
...
void GSM_poweron(void)
{
    ...
    io_conf.pin_bit_mask = (1<<GPIO_GSMPWR);
